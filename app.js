// ═══════════════════════════════════════════════════
// IZVPRO app.js — v1.2 (sync sa DB shemom)
// ═══════════════════════════════════════════════════
...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.warn('[IZVPRO] Supabase config nije postavljen.');
}

const { createClient } = supabase;
const db = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

// ─── KONSTANTE — usklađene sa DB check constraints ────────────────────────────
...
const DOCUMENT_BASIS_TYPE_OPTIONS = [
  'izvrsna_isprava', 'verodostojna_isprava', 'drugo'
];

// PATCH: usklađeno sa case_deadlines.deadline_type check constraint
...
// PATCH: usklađeno sa profiles.role check constraint
const PROFILE_ROLE_OPTIONS = [
  'admin', 'izvrsitelj', 'referent', 'dostavljac', 'poverilac'
];
...
function logError(context, err) {
  if (isDev) console.error(`[IZVPRO][${context}]`, err);
}
...
function getErrorMessage(err) {
  ...
  if (msg.toLowerCase().includes('row-level security')) return 'RLS blokira ovu akciju. Provjeri policy u Supabase.';
  if (msg.toLowerCase().includes('does not exist')) return 'Tražena funkcija ili tabela ne postoji u bazi.';
  if (msg.toLowerCase().includes('uq_case_deadlines_open_unique')) return 'Za ovaj predmet već postoji otvoren rok istog tipa.';
  if (msg.toLowerCase().includes('case_deadlines_deadline_type_check')) return 'Izaberi dozvoljen tip roka iz liste.';
  if (msg.toLowerCase().includes('cases_document_basis_type_check')) return 'Izaberi dozvoljen tip isprave iz liste.';
...
async function loadCurrentProfile() {
  try {
    ...
    if (error) {
      ...
      // Fallback: konstruiši minimalni profil iz auth metadata
      _currentProfile = {
        id: user.id,
    ...
    if (!data) {
      dbg('Profil nije pronađen za userId: ' + user.id);
      return null;
    }

    _currentProfile = data;
    dbg('Profil učitan: ' + data.email + ' / ' + data.role);
    return _currentProfile;
  } catch (err) {
...
async function loadDashboard() {
  try {
    ...
    // KPI kartice
    const setKpi = (id, metricVal, metaText) => {
      const metric = document.getElementById(id);
      const meta = document.getElementById(id + '-meta');
      if (metric) metric.textContent = formatNumber(metricVal);
...
function ensureCasesWorkspace() {
  ...
  if (!viewEl) return;
  if (document.getElementById('cases-workspace')) return;
...
function renderCasesList(items) {
  ...
  list.innerHTML = items.map(item => {
    const id = item.id || '';
    const number = item.case_number || '-';
    const title = item.title || 'Bez naziva';
    const status = item.status || 'prijem';
    const amount = item.claim_amount;
    const updated = item.last_status_changed_at || item.updated_at || item.created_at;
    ...
    return `
      <div class="case-row ${isActive ? 'active' : ''}" data-case-id="${id}">
        ...
        <div class="case-meta">
          ${amount != null ? `<span class="chip">${formatCurrency(amount)}</span>` : ''}
          ${updated ? `<span class="chip">${formatDateOnly(updated)}</span>` : ''}
        </div>
      </div>`;
  }).join('');
...
function wireCasesToolbar() {
  ...
  search?.addEventListener('input', () => renderCasesList(getFilteredCases()));
  filter?.addEventListener('change', () => renderCasesList(getFilteredCases()));
}
...
async function loadSelectedCaseDetail(caseId) {
  ...
  try {
    // Uzimamo direktno iz cases tabele + case_parties + case_deadlines + case_events
    ...
    detail.innerHTML = `
      <div class="detail-grid">
        <div class="detail-box">
          <h4>Broj predmeta</h4>
          <p>${safe(c.case_number)}</p>
        </div>
        <div class="detail-box">
          <h4>Status</h4>
          <p><span class="status ${statusClass(c.status)}">${safe(c.status)}</span></p>
        </div>
        <div class="detail-box">
          <h4>Poverilac</h4>
          <p>${safe(creditor?.full_name)}</p>
        </div>
        <div class="detail-box">
          <h4>Dužnik</h4>
          <p>${safe(debtor?.full_name)}</p>
        </div>
        <div class="detail-box">
          <h4>Iznos potraživanja</h4>
          <p>${c.claim_amount != null ? formatCurrency(c.claim_amount) : '-'}</p>
        </div>
        <div class="detail-box">
          <h4>Datum prijema</h4>
          <p>${formatDateOnly(c.received_at)}</p>
        </div>
      </div>
    ...
    detail.innerHTML = `<div class="empty-note" style="color:var(--danger);">Greška pri učitavanju: ${getErrorMessage(err)}</div>`;
  }
}
...
async function handleNewCaseSubmit(e) {
  ...
  // PATCH: polje je 'notes' u DB (ne 'note')
  const payload = {
    tenant_id: tenantId,
    ...
    notes: form.notes.value.trim() || null   // ← ISPRAVKA: 'notes' ne 'note'
  };
...
function openNewDeadlineModal(caseId) {
  ...
  document.getElementById('new-deadline-form')?.addEventListener('submit', async e => {
    ...
    const payload = {
      ...
      created_by: _currentProfile.id,
      title,
      deadline_type: form.deadline_type.value,
      due_date: dueDate,
      status: 'open',
...
function wireLoginForm() {
  ...
  const noProfileEl = document.getElementById('login-no-profile');
  ...
  form.addEventListener('submit', async e => {
    ...
    if (!email || !password) {
      errorEl.textContent = 'Unesite email i lozinku.';
    ...
    noProfileEl?.classList.remove('visible');

    try {
      const { error } = await db.auth.signInWithPassword({ email, password });
      if (error) throw error;
      // onAuthStateChange preuzima od ovde
    } catch (err) {
  ...
  db.auth.onAuthStateChange(async (event, session) => {
    ...
    if (event === 'SIGNED_IN' || event === 'INITIAL_SESSION') {
      ...
      if (!profile.is_active) {
        document.getElementById('login-error').textContent = 'Vaš nalog je deaktiviran.';
