...
export default async function PredmetDetailPage({ params }: PageProps) {
  ...
  // Dohvati predmet — tenant filter obavezan
  ...
  // Dohvati rokove za ovaj predmet
  ...
  const infoRows: { label: string; value: React.ReactNode }[] = [
    ...
    { label: 'Rok sledeće radnje', value: p.rok_sledece_radnje
              ...
              <ul style={{ listStyle: 'none', padding: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {(rokovi as Rok[]).map((r) => {
                  ...
                  const daysLabel = isPast
                    ? `Prošao (${Math.abs(days)} d. ago)`
                    : days === 0 ? 'Danas' : days === 1 ? 'Sutra' : `Za ${days} d.`;
                  const daysColor = isPast
                    ? 'var(--color-text-faint)'
                    : days === 0 ? 'var(--color-error)'
                    : days === 1 ? 'var(--color-warning)'
