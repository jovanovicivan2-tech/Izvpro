...
export default async function PredmetiPage() {
      ...
      <div>
        ...
        >
          <p className="text-sm" style={{ color: 'var(--color-error)' }}>
            Greška pri učitavanju predmeta: {error.message}
          </p>
        </div>
      </div>
    );
  ...
  const ukupnoAktivnih = predmeti?.filter((p: Predmet) => p.status === 'aktivan').length ?? 0;
  const ukupnoRokovaUskoro = predmeti?.filter((p: Predmet) => isRokUskoro(p.rok_sledece_radnje)).length ?? 0;
