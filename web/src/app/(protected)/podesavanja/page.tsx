...
export default async function PodesavanjaPage({ searchParams }: PageProps) {
  ...
  const errorMsg =
    ...
    error === 'wrong_password'      ? 'Stara lozinka nije ispravna.' :
    error === 'mismatch'            ? 'Nove lozinke se ne podudaraju.' :
    error === 'too_short'           ? 'Nova lozinka mora imati najmanje 8 znakova.' :
    error === 'server'              ? 'Greška na serveru. Pokušajte ponovo.' :
  ...
  const successMsg =
    ...
    success === 'lozinka'   ? 'Lozinka je uspešno promenjena.' :
    null;
  ...
  return (
    <div style={{ maxWidth: 800 }}>
      <div className="mb-5">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>Kancelarija, korisnici i nalog</p>
      </div>

      {/* Tabovi */}
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        <Link href="/podesavanja" style={tabStyle('kancelarija')}>Kancelarija</Link>
        <Link href="/podesavanja?tab=korisnici" style={tabStyle('korisnici')}>Korisnici ({korisnici?.length ?? 0})</Link>
        <Link href="/podesavanja?tab=lozinka" style={tabStyle('lozinka')}>Promena lozinke</Link>
      </div>
      ...
      {activeTab === 'lozinka' && (
        <div style={{ maxWidth: 480 }}>
          <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <div className="px-5 py-3.5" style={{ borderBottom: '1px solid var(--color-border)' }}>
              <p className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Promena lozinke</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>Unesite trenutnu lozinku i novu lozinku</p>
            </div>
            <div className="p-5">
              <form method="POST" action="/api/lozinka" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
